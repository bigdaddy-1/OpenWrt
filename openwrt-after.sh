...
# Modify default attendedsysupgrade
...
# sed -i '24d' ./package/luci-data/applications/luci-app-zerotier/luasrc/model/cbi/zerotier/manual.lua
sed -i 's/\.\.\/\.\./$(TOPDIR)\/feeds\/luci/g' ./package/luci-data/applications/luci-app-zerotier/Makefile
# sed -i 's/ +jsonfilter +ucode//g' ./package/luci-data/applications/luci-app-zerotier/Makefile
sed -i 's/START=99/START=97/g' ./package/luci-data/applications/luci-app-zerotier/root/etc/init.d/zerotier

# Modify default openclash
sed -i 's/+unzip//g' ./package/luci-app-openclash/Makefile
sed -i 's/nft add rule inet fw4 openclash_mangle_output skuid \!= 65534 udp dport { 0-65535 } ip daddr {198.18.0.0\/16} meta mark set "\$PROXY_FWMARK" 2>\/dev\/null//g' ./package/luci-app-openclash/root/etc/init.d/openclash
sed -i 's/nft add rule inet fw4 openclash_dns_hijack meta nfproto {ipv4} tcp dport 53 mark set "\$PROXY_FWMARK" comment \\"OpenClash TCP DNS Hijack\\" 2>\/dev\/null/nft add rule inet fw4 openclash_dns_hijack meta nfproto {ipv4} tcp dport 53 mark set "\$PROXY_FWMARK" comment \\"OpenClash TCP DNS Hijack\\" 2>\/dev\/null\n         nft add rule inet fw4 openclash_mangle_output skuid \!= 65534 udp dport { 0-65535 } meta mark set "\$PROXY_FWMARK" 2>\/dev\/null\n         nft flush chain inet fw4 mangle_postrouting 2>\/dev\/null\n         nft add rule inet fw4 mangle_postrouting oifname ztj32b5oay counter return 2>\/dev\/null/g' ./package/luci-app-openclash/root/etc/init.d/openclash

# Modify default sing-box
curl -LOs https://github.com/haishanh/yacd/archive/gh-pages.zip
...
if [ "$singbox_prefor_normalized" \> "$singbox_latest_normalized" ]; then
...
sed -i "s/PKG_VERSION:=.*/PKG_VERSION:=$singbox_version/g" ./package/sing-box/Makefile
sed -i "s/PKG_HASH:=.*/PKG_HASH:=$singbox_hash/g" ./package/sing-box/Makefile
...
if [ "$mosdns_prefor_normalized" \> "$mosdns_latest_normalized" ]; then
...
plugins:
  ...
  - tag: 'hosts'
    ...
    args:
      entries:
        - domain:.lan 10.1.1.10 10.1.1.11 10.1.1.12
