...
NFT_EOF
    if [ "$SELECTED_MODE" == "tproxy" ]; then
        ...
        ct mark 0xff meta l4proto { tcp,udp } mark set "$FWMARK_POLICY_ROUTING" tproxy to :"$TPROXY_REDIRECT_PORT"
NFT_EOF
    elif [ "$SELECTED_MODE" == "tun" ]; then
        cat >>"$NFTFILE" <<NFT_EOF
...
NFT_EOF
    ...
    chain nat_optnat {
        ...
        fib daddr type local meta l4proto { tcp, udp } th dport 53 redirect to :"$DNS_REDIRECT_PORT"
NFT_EOF
    if [ "$SELECTED_MODE" == "tun" ]; then
        cat >>"$NFTFILE" <<NFT_EOF
...
NFT_EOF
    ...
    chain nat_dstnat {
        ...
        meta l4proto { tcp, udp } th dport 53 redirect to :"$DNS_REDIRECT_PORT"
NFT_EOF
    if [ "$SELECTED_MODE" == "tun" ]; then
        cat >>"$NFTFILE" <<NFT_EOF
        iifname "$ZT_INTERFACE" return
        iifname "$TUN_INTERFACE" return
...
NFT_EOF
    ...
    chain nat_redir {
        meta l4proto tcp redirect to :"$TUN_REDIRECT_PORT"
    }

    chain input {
        type filter hook input priority filter; policy accept;
        iifname "$TUN_INTERFACE" accept comment "\Allow traffic from TUN to router\"
    }

    chain forward {
        type filter hook forward priority filter; policy drop;
        oifname "$TUN_INTERFACE" accept comment "\Sing-box TUN Forward\"
        iifname "$TUN_INTERFACE" accept comment "\Sing-box TUN Forward\"
    }
NFT_EOF
    fi
