...
export TPROXY_REDIRECT_PORT=8012
...
start_service() {
    ...
    while ! ping -c 1 -W 1 "${target_ip}" > /dev/null 2>&1; do
        if [ "$attempt" -ge "$max_attempts" ]; then
            echo "ERROR: Failed to connect to ${target_ip} after ${max_attempts} attempts. Exiting.."
            exit 1
        fi
        echo "Attempt ${attempt}/${max_attempts} to connect to ${target_ip} failed.."
        attempt=$((attempt + 1))
        sleep 5
    done
...
stop_service() {
    ...
    if [[ -n "$nft_file" ]]; then
        SELECTED_MODE=$(basename "$nft_file" | sed -E 's/singbox-(.*)\.nft/\1/')
    else
        SELECTED_MODE="unknown"
    fi
    
    echo "SING-BOX ($SELECTED_MODE) service has been stopped.."
