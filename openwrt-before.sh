...
start_service() {
    check_connectivity
    procd_open_instance
    procd_set_param command capsh --caps="${capabilities}+eip" -- -c "capsh --user=nobody --addamb='${capabilities}' -- -c '$APPBINARY start -d $DIRECTORY -c $CONFIGFILE'"
...
reload_service() {
    echo "Reloading MOSDNS service..."
    stop_service
    start_service
    echo "MOSDNS service reloaded."
}
