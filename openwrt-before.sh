...
check_config_update() {
    ...
    while [ $retries -lt $max_retries ]; do
        remote_content_raw=$(curl -fsL --connect-timeout 5 "$CONFIG_URL")
    ...
    if [ -f "$CONFIGFILE" ]; then
        ...
        if [ "$local_sha256" != "$remote_sha256" ]; then
            ...
            echo "$remote_content_modified" >"$CONFIGFILE"
        else
...
NFT_EOF
    ...
    chain nat_optnat {
        ...
        tcp dport 53 ip6 daddr {::1} meta skgid != "$SKG_ID" counter redirect to :"$DNS_REDIRECT_PORT"
        udp dport 53 ip6 daddr {::1} meta skgid != "$SKG_ID" counter redirect to :"$DNS_REDIRECT_PORT"
NFT_EOF
    if [ "$SELECTED_MODE" == "tun" ]; then
        cat >>"$NFTFILE" <<NFT_EOF
        skgid == "$SKG_ID" return
...
chown dnsmasq:nogroup "$APPBINARY" 2>/dev/null
...
check_files() {
   ...
   local ip_url="https://raw.githubusercontent.com/Loyalsoldier/geoip/release/text/"
    ...
    download_file() {
        ...
        if [ ! -f "$path" ] || [ "$(curl -fsL --connect-timeout 5 "SURL2/convert/_start_/$url$file/_end_/$file?type=plain-text&target=plain-text&del=true" | md5sum)" != "$(cat "$path" | md5sum)" ]; then
            curl -fsL --connect-timeout 5 -o "$path" "SURL2/convert/_start_/$url$file/_end_/$file?type=plain-text&target=plain-text&del=true"
            return $?
        else
            return 0
        fi
...
start_service() {
    check_connectivity
    procd_open_instance
    procd_set_param command capsh --caps="${capabilities}+eip" -- -c "capsh --user=dnsmasq --addamb='${capabilities}' -- -c '$APPBINARY start -d $DIRECTORY -c $CONFIGFILE'"
