...
setup_server_firewall() {
    ...
    if [ "$selected_mode" == "tproxy" ]; then
        ...
        if [ -z "$(nft list chain inet fw4 mangle_prerouting | grep tproxy)" ]; then
            ...
            nft 'add rule inet fw4 mangle_prerouting meta mark 0x1a counter return'
            ...
            nft 'add rule inet fw4 optnat tcp dport 53 ip daddr {127.0.0.1} meta skuid != 65534 counter redirect to :5053'
            nft 'add rule inet fw4 optnat udp dport 53 ip daddr {127.0.0.1} meta skuid != 65534 counter redirect to :5053'
            nft "add rule inet fw4 dstnat udp dport 53 counter redirect to :5053"
            nft "add rule inet fw4 dstnat tcp dport 53 counter redirect to :5053"
...
setdown_server_firewall() {
    ...
    if [ -n "$(nft list chain inet fw4 forward | grep tun0)" ]; then
        ...
        sleep 3
        ip tuntap del mode tun name tun0 2>"/dev/null" || true
}
