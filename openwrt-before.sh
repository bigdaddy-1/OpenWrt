...
NFT_EOF
    ...
    chain nat_pstnat {
        ...
        meta nfproto { ipv4, ipv6 } oifname tun0 return comment "\Disable NAT for TUN IPv4 & IPv6\"
    }
    ...
    chain input {
        ...
        iifname tun0 counter accept comment "\Allow traffic from TUN to router\"
    }

    chain forward {
        type filter hook forward priority filter; policy drop;
        meta l4proto { tcp, udp } oifname tun0 counter accept comment "\Sing-box TUN Forward\"
        meta l4proto { tcp, udp } iifname tun0 counter accept comment "\Sing-box TUN Forward\"
    }
NFT_EOF
    fi
