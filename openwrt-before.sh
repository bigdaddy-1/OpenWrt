...
setup_server_firewall() {
    ...
    if [ "$selected_mode" == "tproxy" ]; then
        ...
        if [ -z "$(nft list chain inet fw4 mangle_prerouting | grep tproxy)" ]; then
            ...
            nft 'add chain inet fw4 optnat { type nat hook output priority 0; }'
        ...
        if [ -z "$(nft list chain inet fw4 forward | grep tun0)" ]; then
            ...
            ip rule add fwmark 255 lookup main
            ip route replace default dev tun0 table 101
            ip -6 rule add fwmark 101 lookup 2022
            ip -6 rule add fwmark 255 lookup main
        fi
    fi
}
...
clean_rules() {
    ...
    ip -6 rule del table 101 2>"/dev/null"
...
check_files() {
    files="direct-list.txt apple-cn.txt"
    files_url="https://fastly.jsdelivr.net/gh/Loyalsoldier/v2ray-rules-dat@release/"
    ip_cidr="cn.txt private.txt"
    ip_url="https://fastly.jsdelivr.net/gh/Loyalsoldier/geoip@release/text/"
    ...
    download_file() {
        ...
        if [ ! -f "$path" ] || [ "$(curl -fsL --connect-timeout 3 "$url$file" | md5sum)" != "$(cat "$path" | md5sum)" ]; then
            curl -fsL --connect-timeout 3 -o "$path" "$url$file"
            return $?
        else
            return 0
        fi
