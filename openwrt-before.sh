...
NFT_EOF
    ...
    chain nat_optnat {
        ...
        meta l4proto { tcp, udp } th dport 53 ip daddr 127.0.0.1 counter redirect to :"$DNS_REDIRECT_PORT"
        meta l4proto { tcp, udp } th dport 53 ip6 daddr ::1 counter redirect to :"$DNS_REDIRECT_PORT"
NFT_EOF
    if [ "$SELECTED_MODE" == "tun" ]; then
        cat >>"$NFTFILE" <<NFT_EOF
